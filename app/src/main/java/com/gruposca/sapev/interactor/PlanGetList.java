...
public class PlanGetList extends AbsInteractor<List<PlanResult>>{

    private final PlanRepository planRepository;

    @Inject
    public PlanGetList(ThreadExecutor threadExecutor, PostThreadExecutor postThreadExecutor, PlanRepository planRepository) {
        super(threadExecutor, postThreadExecutor);
        this.planRepository = planRepository;
    }

    public void execute(Callback<List<PlanResult>> callback) {
    ...
    public void run() {
        try {
            notifyResponse(planRepository.getPlans());
        } catch (Exception e) {
            notifyError(e);
        }
    }
