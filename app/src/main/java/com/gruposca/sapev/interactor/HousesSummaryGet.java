...
public class HousesSummaryGet extends AbsInteractor<HousesSummaryQuery> {

    private final VisitsSummaryRepository visitsSummaryRepository;

    @Inject
    public HousesSummaryGet(ThreadExecutor threadExecutor, PostThreadExecutor postThreadExecutor, VisitsSummaryRepository visitsSummaryRepository) {
        super(threadExecutor, postThreadExecutor);
        this.visitsSummaryRepository = visitsSummaryRepository;
    }

    public void execute(Callback<HousesSummaryQuery> callback) {
    ...
    public void run() {
        try {
            notifyResponse(visitsSummaryRepository.getHousesSummary());
        } catch (Exception e) {
            notifyError(e);
        }
    }
