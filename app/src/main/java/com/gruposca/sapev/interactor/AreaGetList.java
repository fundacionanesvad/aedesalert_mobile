...
public class AreaGetList extends AbsInteractor<List<AreaListQuery>> {

    private final AreaRepository areaRepository;

    @Inject
    public AreaGetList(ThreadExecutor threadExecutor, PostThreadExecutor postThreadExecutor, AreaRepository areaRepository) {
        super(threadExecutor, postThreadExecutor);
        this.areaRepository = areaRepository;
    }

    public void execute(Callback<List<AreaListQuery>> callback) {
    ...
    public void run() {
        try {
            notifyResponse(areaRepository.getAreas());
        } catch (Exception e) {
            notifyError(e);
        }
    }
