...
public class VisitSave extends AbsInteractor<Void> {
    ...
    @Inject
    public VisitSave(ThreadExecutor threadExecutor, PostThreadExecutor postExecutionThread, VisitRepository visitRepository, HouseRepository houseRepository) {
    ...
    public void run() {
        try {
            ...
            house.visitInProgress = true;
            if (visit.result.id == Visit.RESULT_INSPECTION) {
